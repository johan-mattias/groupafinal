...
// This is encoded as JSON and returned.
...
$myArray = array();
  if ($result = $conn->query("SELECT cats.name, tags.tag FROM cats JOIN catsTagsMap ON cats.id = catsTagsMap.cat_id JOIN tags ON tags.id = catsTagsMap.tag_id WHERE tags.Tag = '.$data['tag'].';")) {
    $tempArray = array();
